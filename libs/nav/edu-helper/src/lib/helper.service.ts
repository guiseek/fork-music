...
@Injectable()
export class HelperService<T extends IStepOption = IStepOption> {
  ...
  public anchors: { [anchorId: string]: HelperAnchorDirective } = {};
  private status: HelperState = HelperState.OFF;
  ...
  public pause(): void {
    this.status = HelperState.PAUSED;
    this.hideStep(this.currentStep);
    this.pause$.next();
  }
  ...
  public register(anchorId: string, anchor: HelperAnchorDirective): void {
